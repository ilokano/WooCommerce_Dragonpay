        ...
        function check_dragonpay_response_returnurl() {
            ...
            */
            ...
            if ($status == 'S') {
                ...
                $order->update_status('pending', sprintf(__('Payment %s via dragonpay.', 'woocommerce'), $ref_no ) );
                wp_redirect($order->get_checkout_order_received_url());
                exit;
            }
            else if ($status == "F") { 
                $order->add_order_note('dragonpay Payment Status: FAILED');
                $order->update_status('failed', sprintf(__('Payment %s via dragonpay.', 'woocommerce'), $ref_no ) );
                wp_redirect($order->get_cancel_order_url());
                exit;
            } 
            else  {
                $order->add_order_note('dragonpay Payment Status: Invalid Transaction');
                $order->update_status('on-hold', sprintf(__('Payment %s via dragonpay.', 'woocommerce'), $ref_no ) );
                wp_redirect($order->get_cancel_order_url());
                exit;
            }	
        }
